...
class Sensitivity(object):
    ...
    def __init__(self, min_ratio, max_ratio, num, metric, round, exp, topk, *args):
        ...
        self.exp = exp
        self.topk = topk
        self.inputsize = args[0].img_size
        self.logger = args[1]
        self.round = round
    ...
    def set_group(self, model):
        ...
        # self.groups = [[f'model[{i}].conv{n}.conv' for n in
                        # range(1, 5)] for i in bottleneck_index]
        self.groups = []
    ...
    def __call__(self, model, dataloader, part, sensitivity=None):
        ...
        temp_m = model.cuda()
        temp_m.eval()
        example_inputs = torch.randn(1, 3, self.inputsize[0], self.inputsize[1])
    ...
    def get_ratio(self, sensitivity):
        ...
        for k in sens_keys:
            if len(sensitivity[k]['loss']) > 1:
                DG.build_dependency(pruned_model, example_inputs=torch.randn(1, 3, self.inputsize[0], self.inputsize[1]))
