...
class VoVPDCSP2(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.gsb = nn.Sequential(*(PDBottleneck(c_, c_, e=1.0) for _ in range(n)))
...
class Yolov7_Tiny_E_ELAN(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        ...
        self.conv1 = Conv(inc, hidc, k=1, act=act)
        self.conv2 = Conv(inc, hidc, k=1, act=act)
        # self.conv3 = PDConv(hidc, hidc,k=3,s=1,act=act)
        self.conv3 = PDBottleneck(hidc, hidc,k=3,s=1)
        self.conv4 = Conv(hidc, hidc, k=3, act=act)
        self.conv5 = Conv(hidc * 4, ouc, k=1, act=act)
...
class PConv(nn.Module):
    def __init__(self, dim, ouc, n_div=4, forward='split_cat'):
        ...
        self.partial_conv3 = DCNv2(in_channels=self.dim_conv3, out_channels=self.dim_conv3, kernel_size=3, stride=1)
...
class Yolov7_E_ELAN_PConv(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_E_ELAN_PConv, self).__init__()

        self.conv1 = Conv(inc, hidc, k=1, act=act)
        self.conv2 = Conv(inc, hidc, k=1, act=act)
        # self.conv1 = PConv(inc, hidc)
        # self.conv2 = PConv(inc, hidc)
        self.conv3 = PConv(hidc, hidc)
        self.conv4 = PConv(hidc, hidc)
        self.conv5 = PConv(hidc, hidc)
        self.conv6 = PConv(hidc, hidc)
        # self.conv7 = PConv(hidc * 4, ouc)
...
class PD_ELAN(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(PD_ELAN, self).__init__()

        self.conv1 = Conv(inc, hidc, k=1, act=act)
        self.conv2 = Conv(inc, hidc, k=1, act=act)
        # self.conv1 = PConv(inc, hidc)
        # self.conv2 = PConv(inc, hidc)
        # self.conv3 = PDConv(hidc, hidc,k=3,s=1)
        self.conv3 = PDBottleneck(hidc, hidc, k=3, s=1)
        self.conv4 = Conv(hidc, hidc,k=3,s=1)
        # self.conv5 = PDConv(hidc, hidc,k=3,s=1)
        self.conv5 = PDBottleneck(hidc, hidc, k=3, s=1)
        self.conv6 = Conv(hidc, hidc,k=3,s=1)
        # self.conv7 = PConv(hidc * 4, ouc)
        self.conv7 = Conv(hidc * 4, ouc, k=1, act=act)
...
class Yolov7_E_ELAN_DCNV(nn.Module):
    def __init__(self, inc, ouc, hidc, act=True):
        super(Yolov7_E_ELAN_DCNV, self).__init__()

        self.conv1 = Conv(inc, hidc, k=1, act=act)
        self.conv2 = Conv(inc, hidc, k=1, act=act)
        # self.conv3 = DCNv2(hidc, hidc, kernel_size=3, stride=1, padding=1, act=act)
        # self.conv4 = DCNv2(hidc, hidc, kernel_size=3, stride=1, padding=1, act=act)
        # self.conv5 = DCNv2(hidc, hidc, kernel_size=3, stride=1, padding=1, act=act)
        # self.conv6 = DCNv2(hidc, hidc, kernel_size=3, stride=1, padding=1, act=act)
        # self.conv3 = DCNv2(hidc, hidc, kernel_size=3, stride=1, padding=1, act=act)
        # self.conv4 = PConv(hidc, hidc)
        self.conv3 = PConv(hidc, hidc)
        self.conv4 = DCNv2(hidc, hidc, kernel_size=3, stride=1, padding=1, act=act)
    ...
    def forward(self, x):
        ...
        x5 = self.conv5(x1)
        x6 = self.conv6(x5)
        x_concat = torch.concat([x1, x3, x4, x6], dim=1)
        x_final = self.conv7(x_concat)
        return x_final
...
#         self.conv1 = Conv(inc, ouc, k=1, act=act)
#         self.conv2 = Conv(inc, ouc, k=1, act=act)
#         self.conv3 = DCNv2(ouc, hidc, kernel_size=3, stride=1, padding=1, act=act)
#         #self.conv3 = PConv(ouc, hidc)
#         self.conv4 = PConv(hidc, hidc)
#         #self.conv4 = DCNv2(hidc, hidc, kernel_size=3, stride=1, padding=1, act=act)
#         self.conv5 = PConv(ouc, hidc)
#         self.conv6 = DCNv2(hidc, hidc, kernel_size=3, stride=1, padding=1, act=act)
...
class PDConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        ...
        c_ = c2 // 2
        self.cv1 = Conv(c1, c_, k, s, p, g, act)
        self.cv2 = PConv(c_, c_)  # 修改为PConv
        # self.cv2 = Conv(c_, c_, 5, 1, p, c_, act)
        # self.cv1 = PConv(c1, c_, k, s, p, g, act)  # 修改为PConv
        # self.cv2 = DCNv2(c_, c_, kernel_size=5, stride=1, padding=p, act=act)  # 修改为DCNv2
        # self.cv1 = PConv(c1, c_)  # 不传递额外参数
        # self.cv2 = DCNv2(c_, c_, kernel_size=3, stride=1, padding=p, act=act)

    def forward(self, x):
        x1 = self.cv1(x)
        x2 = torch.cat((x1, self.cv2(x1)), 1) #torch.concat([x1, x2, x4, x6], dim=1)#这一行考虑的是一半是PConv+一半是经过P后的D处理
        # x2 = torch.cat((x1, self.cv2(x)), 1)#这一行考虑DCNV单独对一半的特征图进行处理拼接
        # shuffle
        # y = x2.reshape(x2.shape[0], 2, x2.shape[1] // 2, x2.shape[2], x2.shape[3])
        # y = y.permute(0, 2, 1, 3, 4)
        # return y.reshape(y.shape[0], -1, y.shape[3], y.shape[4])
...
class PDBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1, e=0.5):
        ...
        self.conv_lighting = nn.Sequential(
            PDConv(c1, c_, 1, 1),
            PDConv(c_, c2, 3, 1, act=False))
            # PConv(c1, c_, 1, 1),  # 替换为PConv
            # DCNv2(c_, c2, kernel_size=3, stride=1, padding=1, act=False)  # 替换为DCNv2
        self.shortcut = Conv(c1, c2, 1, 1, act=False)
